package de.mika32.finaltest.commands;
...
public class TeleporterCommand implements CommandExecutor {
    ...
    public static void sendList(CommandSender sender){
        ...
        if(names.length == 1){
            classicErrorMessage(sender, "Error with List command", "There are no Tp_Blocks which could have a name");
            return;
        }
    ...
    public static void sendAktivMessages(CommandSender sender){
        // Deaktivierung der Nachrichten zwischen den Tp durch die TpBlöcke! Somit werden die Tp_Blocke zu einem instant Tp!
        // Übergabe von true false Parameter für die Aktivierung und Deaktiverung der Nachrichten
    ...
    public static void sendAliases(CommandSender sender, String page){
        ...
        sender.sendMessage(ChatColor.YELLOW.toString() + "-----------------" + ChatColor.WHITE.toString() + " All of the Aliases " + ChatColor.YELLOW.toString() + "-------------------");
...
// Zufünftige Features:
//                      - restliche Methoden fertig schreiben
//                      - Einstellung eines Preises für den Tp!
...
//                      - performacne von listenern verbessern
//                      - save von erstellten Tp_Blocks in config
//                      - Tab auto complete für Commands
//                      - eigenes Plugin nur für Tp_Blocks machen
